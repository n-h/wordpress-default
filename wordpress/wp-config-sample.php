...
$basePath = !empty($_SERVER['SCRIPT_NAME']) ? dirname($_SERVER['SCRIPT_NAME']) : ''; // Start to calculate base of WP_HOME
if(substr($basePath, -18) == 'wordpress/wp-admin') $basePath = dirname(dirname($basePath)); // If in wp-admin perform ../../ on $basePath
if(php_sapi_name() === 'cli') $basePath = !empty($_SERVER['REQUEST_URI']) ? dirname($_SERVER['REQUEST_URI']) : ''; // Override $basePath if running from cli
